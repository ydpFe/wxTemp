    ...
    <div>
        ...
        <div class="borderTop marginTop_10">
            <template v-for="data in colorObj">
                <div class="selectMsg borderBottom fontSize_15" @click="setIn" :value="data.id">{{data.color_name}}</div>
            </template>
        </div>
        <router-link :to="{ name: 'defindInColor'}" class="selectMsg paddingTop_20">
...
export default{
    ...
    mounted(){
        ...
        this.$http.get(apiHost+'/car/color_series_list',{params: {series_id: this.seriesId}}).then(res => {
            if (res.data.code === 0) {
                this.colorObj = res.data.data.in;
    ...
    methods:{
         ...
         */
        setIn(event){
            this.$store.commit('setIn',event.target.outerText);
            this.$router.go(-1)
        }
    }
}
